...
export const api = {
  ...
  // Проводки мимо документов и сторно — и проверка, не считаем ли мы
  // документ, который 1С уже сняла.
  manualEntries: () => request('/manual-entries'),
  // Подотчёт: сколько денег на руках у сотрудников и на что потрачены.
  ...
  salesdocHiddenOrdersProbe: ({ sd_id, code_1c, number, withAgents = true }) => {
    const qs = new URLSearchParams()
    if (sd_id) qs.set('sd_id', sd_id)
    if (code_1c) qs.set('code_1c', code_1c)
    if (number) qs.set('number', number)
    if (!withAgents) qs.set('with_agents', 'false')
